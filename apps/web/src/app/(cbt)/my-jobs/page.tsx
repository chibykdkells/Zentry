...
function MetricCard({
  ...
  return (
    <article className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-[#0D1B3E] md:h-11 md:w-11 md:rounded-2xl">
        <Icon size={16} />
      </div>
      <p className="mt-3 text-2xl font-bold tracking-tight text-slate-900 md:mt-5 md:text-3xl">{value}</p>
      <p className="mt-0.5 truncate text-xs text-slate-500 md:mt-1 md:text-sm">{title}</p>
    </article>
  );
}
